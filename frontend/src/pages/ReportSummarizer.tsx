...
const getFileIcon = (file: File) => {
	...
	if (file.type.startsWith("audio/"))
		return <Mic className="h-5 w-5 text-purple-600 mr-2" />;
	if (file.type === "application/pdf")
		return <FileText className="h-5 w-5 text-red-500 mr-2" />;
	if (file.type.startsWith("text/"))
		return <FileX className="h-5 w-5 text-orange-600 mr-2" />;
...
const ReportSummarizer = () => {
	...
	const handleAnalyzeFiles = async () => {
		...
		uploadedFiles.forEach((file) => {
			formData.append("files", file); // "files" should match your backend field name
		});
		try {
			const response = await axios.post("/api/reportanalysis/", formData, {
									...
									{uploadedFiles.map((file, idx) => (
										<li
											...
											<div className="flex items-center">
												{getFileIcon(file)}
												<span className="text-gray-800 text-sm truncate max-w-xs">
													{file.name}
												</span>
											</div>
											<button
						...
						{analysisResult ? (
							<div className="space-y-6">
								...
								<div>
									...
									<ul className="space-y-2">
										{analysisResult.keyFindings.map(
